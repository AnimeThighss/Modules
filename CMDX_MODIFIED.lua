...
AimFor = "Head"
local Police = {"Dark blue"}
local Criminal = {"Dark indigo", "Bright red", "Gold", "Really blue"}
local LocalPlayer = game.Players.LocalPlayer



function checkTeam(Player)
    local teamColor = tostring(Player.TeamColor)
	local LPteamColor = tostring(LocalPlayer.TeamColor)
	print(teamColor)
	if table.find(Criminal, teamColor) and  table.find(Police, LPteamColor) then
		return true
	elseif table.find(Police, teamColor) and table.find(Criminal, LPteamColor) then
		return true
	end
end
...
function GetPlayer()
	...
	if Teamed then
		for i,v in pairs(Players1) do
			if v and v.Character ~= nil and checkTeam(v) then
